...
function syncAsanaProject (asanaProject) {
  ...
  return itemSync.getOps(asanaProject, _.cloneDeep(fields))
    ...
    // Save and execute comment ops
    .then(function (commentSyncOpsList) {
      // Note commentSyncOpsList is actually a collection of operation objects, 
      // one per item update.
      this.commentSyncOpsList = commentSyncOpsList;
      return P.map(commentSyncOpsList, function (commentSyncOps) {
        return commentSync.execOps(commentSyncOps);
      });
    })

    // Update coment last sync for every item whose children we diffed,
