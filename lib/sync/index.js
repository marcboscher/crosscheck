    ...
    .then(function () {
      ...
      return P.map(this.itemSyncOps.toDiffChildren, function (changedItem) {
        return commentSync.getOps(changedItem.asana, changedItem.github)
          ...
          .catch (function (e) {
            // We log because we have no mechanism to report
            console.warn("Got error from getOps - " + (e.error ? e.error : e));
            return this;
          });
...
function sync () {
  ...
  return asana.getWorkspaces()
    ...
    .finally(function () {
      // TODO if rate limite hit, set
      // res.rateLimitReachedUntil = _.now() + 120 * 1000;
    });
