 ...
 * @param {Object} [options] - Should contain a "gh.repo" and "gh.onwer" property.
 * @returns {item:Item}
 *
 * @private
 * @static
...
function toItem (issue, options) {
  ...
  if (options["gh.repo"]) {
    it.fields["gh.repo"] = options["gh.repo"];
  }
  if (options["gh.owner"]) {
    it.fields["gh.owner"] = options["gh.owner"];
  }
 ...
 *
 *   - "gh.owner" : "owner name" - required
 *   - "gh.repo" : "repo name" - required
...
function getIssues (options) {
  ...
  urlPath = "repos/" + options["gh.owner"] + "/" + options["gh.repo"] + "/issues";
